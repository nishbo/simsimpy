...
def rk4(prev, right_side, t, dt):
    ...
    k2 = right_side(t+dt/2., *[prev[i]+k1[i]/2. for i in xrange(len(prev))])
    k3 = right_side(t+dt/2., *[prev[i]+k2[i]/2. for i in xrange(len(prev))])
    k4 = right_side(t+dt, *[prev[i]+k3[i] for i in xrange(len(prev))])
